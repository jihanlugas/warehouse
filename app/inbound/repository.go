...
func (r repository) GetViewById(conn *gorm.DB, id string, preloads ...string) (vInbound model.InboundView, err error) {
	...
	err = conn.Where("id = ? ", id).First(&vInbound).Error
	return vInbound, err
}

func (r repository) Page(conn *gorm.DB, req request.PageInbound) (vInbounds []model.InboundView, count int64, err error) {
	query := conn.Model(&vInbounds)
	...
	err = query.Find(&vInbounds).Error
	if err != nil {
		return vInbounds, count, err
	}
