...
// @Accept json
// @Produce json
// @Param req query request.PageStocklog false "url query string"
...
func (h Handler) GetById(c echo.Context) error {
	...
	vStocklog, err := h.usecase.GetById(loginUser, id, preloadSlice...)
	if err != nil {
		return response.Error(http.StatusBadRequest, err.Error(), err, nil).SendJSON(c)
	}

	return response.Success(http.StatusOK, response.SuccessHandler, vStocklog).SendJSON(c)
}
