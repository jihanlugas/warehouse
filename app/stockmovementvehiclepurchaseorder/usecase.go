package stockmovementvehiclepurchaseorder
...
type Usecase interface {
	Page(loginUser jwt.UserLogin, req request.PageStockmovementvehiclePurchaseorder) (vStockmovementvehicles []model.StockmovementvehicleView, count int64, err error)
	GetById(loginUser jwt.UserLogin, id string, preloads ...string) (vStockmovementvehicle model.StockmovementvehicleView, err error)
	Create(loginUser jwt.UserLogin, req request.CreateStockmovementvehiclePurchaseorder) (vStockmovementvehicle model.StockmovementvehicleView, err error)
	Update(loginUser jwt.UserLogin, id string, req request.UpdateStockmovementvehiclePurchaseorder) (vStockmovementvehicle model.StockmovementvehicleView, err error)
	Delete(loginUser jwt.UserLogin, id string) (vStockmovementvehicle model.StockmovementvehicleView, err error)
...
func (u usecase) Page(loginUser jwt.UserLogin, req request.PageStockmovementvehiclePurchaseorder) (vStockmovementvehicles []model.StockmovementvehicleView, count int64, err error) {
	...
	pageReq := request.PageStockmovementvehicle{
		...
		StockmovementvehicleType:   model.StockmovementvehicleTypePurchaseorder,
		Notes:                      req.Notes,
...
func (u usecase) Create(loginUser jwt.UserLogin, req request.CreateStockmovementvehiclePurchaseorder) (vStockmovementvehicle model.StockmovementvehicleView, err error) {
	var tStockmovementvehicle model.Stockmovementvehicle
	var vWarehouse model.WarehouseView
	var tVehicle model.Vehicle
	var vPurchhaseorder model.PurchaseorderView
	...
	if err != nil {
		return vStockmovementvehicle, errors.New(fmt.Sprintf("failed to get %s: %v", u.warehouseRepository.Name(), err))
	}

	if !vWarehouse.IsPurchaseorder {
		return vStockmovementvehicle, errors.New(fmt.Sprint("this warehouse is not allowed to create purchase order"))
	}

	vPurchhaseorder, err = u.purchaseorderRepository.GetViewById(conn, req.PurchaseorderID)
	if err != nil {
		return vStockmovementvehicle, errors.New(fmt.Sprintf("failed to get %s: %v", u.purchaseorderRepository.Name(), err))
	}

	if vPurchhaseorder.PurchaseorderStatus != model.PurchaseorderStatusOpen {
		return vStockmovementvehicle, errors.New(fmt.Sprintf("unable to create purchase order with status %s", strings.ToLower(string(vPurchhaseorder.PurchaseorderStatus))))
	}
	...
	if req.IsNewVehiclerdriver {
		...
		if err != nil {
			return vStockmovementvehicle, errors.New(fmt.Sprintf("failed to create %s: %v", u.vehicleRepository.Name(), err))
		}
	} else {
		tVehicle, err = u.vehicleRepository.GetTableById(conn, req.VehicleID)
		if err != nil {
			return vStockmovementvehicle, errors.New(fmt.Sprintf("failed to get %s: %v", u.vehicleRepository.Name(), err))
		}
	}
	...
	if err != nil {
		return vStockmovementvehicle, errors.New(fmt.Sprintf("failed to create %s: %v", u.stockmovementvehicleRepository.Name(), err))
	}
	...
	vStockmovementvehicle, err = u.stockmovementvehicleRepository.GetViewById(conn, tStockmovementvehicle.ID)
	if err != nil {
		return vStockmovementvehicle, err
	}
...
func (u usecase) Update(loginUser jwt.UserLogin, id string, req request.UpdateStockmovementvehiclePurchaseorder) (vStockmovementvehicle model.StockmovementvehicleView, err error) {
	var tStockmovementvehicle model.Stockmovementvehicle
...
func (u usecase) Delete(loginUser jwt.UserLogin, id string) (vStockmovementvehicle model.StockmovementvehicleView, err error) {
	var tStockmovementvehicle model.Stockmovementvehicle
	...
	if tStockmovementvehicle.StockmovementvehicleStatus != model.StockmovementvehicleStatusLoading {
		return vStockmovementvehicle, errors.New(fmt.Sprintf("unable to delete data with status %s", strings.ToLower(string(tStockmovementvehicle.StockmovementvehicleStatus))))
	}
	...
	if err != nil {
		return vStockmovementvehicle, errors.New(fmt.Sprintf("failed to delete %s: %v", u.stockmovementvehicleRepository.Name(), err))
	}
...
func (u usecase) GenerateDeliveryOrder(loginUser jwt.UserLogin, id string) (pdfBytes []byte, vStockmovementvehicle model.StockmovementvehicleView, err error) {
	...
	vStockmovementvehicle, err = u.stockmovementvehicleRepository.GetViewById(conn, id, "Vehicle", "FromWarehouse", "Product", "Purchaseorder", "Purchaseorder.Customer")
	if err != nil {
...
func NewUsecase(purchaseorderRepository purchaseorder.Repository, stockmovementvehicleRepository stockmovementvehicle.Repository, warehouseRepository warehouse.Repository, stockRepository stock.Repository, stocklogRepository stocklog.Repository, vehicleRepository vehicle.Repository) Usecase {
	return &usecase{
