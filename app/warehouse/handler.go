...
// @Accept json
// @Produce json
// @Param req query request.PageWarehouse false "url query string"
...
func (h Handler) GetById(c echo.Context) error {
	...
	vWarehouse, err := h.usecase.GetById(loginUser, id, preloadSlice...)
	if err != nil {
		return response.Error(http.StatusBadRequest, err.Error(), err, nil).SendJSON(c)
	}

	return response.Success(http.StatusOK, response.SuccessHandler, vWarehouse).SendJSON(c)
}
...
// @Accept json
// @Produce json
// @Param req body request.CreateWarehouse true "json req body"
// @Success      200  {object}	response.Response
// @Failure      500  {object}  response.Response
...
func (h Handler) Create(c echo.Context) error {
	...
	req := new(request.CreateWarehouse)
	if err = c.Bind(req); err != nil {
		return response.Error(http.StatusBadRequest, response.ErrorHandlerBind, err, nil).SendJSON(c)
	}
...
// @Produce json
// @Param id path string true "ID"
// @Param req body request.UpdateWarehouse true "json req body"
// @Success      200  {object}	response.Response
// @Failure      500  {object}  response.Response
...
func (h Handler) Update(c echo.Context) error {
	...
	req := new(request.UpdateWarehouse)
	if err = c.Bind(req); err != nil {
		return response.Error(http.StatusBadRequest, response.ErrorHandlerBind, err, nil).SendJSON(c)
	}
